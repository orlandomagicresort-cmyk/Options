...
# Ensure standard float/int calculation (prevents numpy errors)
                ...
                if "Buy-To-Close" in action_choice:
                    st.markdown("---")
                    st.caption("🧾 **Buy-To-Close**: Buy back this short option to close (fully or partially).")

                    b_c0, b_c1, b_c2, b_c3 = st.columns([1, 1, 1, 1])
                    with b_c0:
                        btc_date = st.date_input("Transaction Date", value=date.today(), key="btc_date")
                    with b_c1:
                        qty_to_process = st.number_input("Qty to Close", min_value=1, max_value=total_avail, value=total_avail, step=1, key="btc_qty")
                    with b_c2:
                        btc_premium = st.number_input("Premium to Buy Back ($)", min_value=0.0, format="%.2f", step=0.01, key="btc_premium")
                    with b_c3:
                        btc_fees = st.number_input("Fees ($)", min_value=0.0, format="%.2f", step=0.01, value=0.0, key="btc_fees")

                    calc_btc = float(btc_premium) * int(qty_to_process) * 100
                    st.write(f"**Cash Effect:** -${calc_btc:,.2f}")
with c_btn:
                    st.write("")
                    st.write("") 
                    if st.button("Process Action", type="primary", use_container_width=True):
                        ...
                        # 3. BUY-TO-CLOSE (close short option)
                            ...
                            st.success(f"Buy-To-Close recorded for {qty_safe} contract(s).")
                            st.cache_data.clear()
                            st.rerun()

# 4. ROLL (BTC then STO; both write to ledger via update_short_option_position)
