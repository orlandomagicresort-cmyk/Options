...
def export_data_page(active_user):
    ...
    # Fetch transactions in smaller date windows to avoid transient httpx.ReadError on large exports
    rows = []
    ...
    while chunk_start <= end_date:
        ...
        rows.extend(_fetch_all(qb, batch_size=80, max_retries=10, retry_base_sleep=1.0))
    ...
    st.divider()
    st.subheader("🧾 Unified Import File")
    st.caption("Generate a CSV in the exact format expected by **Import Data → ✨ Unified Import** (Date, Category, Action, Symbol, Qty, Price, Strike, Expiration, Option Type, Fees).")

    u1, u2 = st.columns(2)
    with u1:
        u_from = st.date_input("From Date (Unified File)", value=start_date, key="unified_from")
    with u2:
        u_to = st.date_input("To Date (Unified File)", value=end_date, key="unified_to")

    if u_from > u_to:
        st.error("From Date must be on or before To Date (Unified File).")
        return

    def _parse_money_after_at(desc: str) -> float:
        m = re.search(r"@\s*\$\s*([0-9,]+(?:\.\d+)?)", str(desc or ""))
        try:
            return float(m.group(1).replace(",", "")) if m else 0.0
        except Exception:
            return 0.0

    def _parse_strike(desc: str) -> float:
        m = re.search(r"\$\s*([0-9]+(?:\.\d+)?)", str(desc or ""))
        try:
            return float(m.group(1)) if m else 0.0
        except Exception:
            return 0.0

    def _parse_expiration(desc: str) -> str:
        # Expect formats like '2026-01-17' or '17-Jan-2026'
        parts = str(desc or "").split()
        # Find the first token that looks like a date after the symbol (best-effort)
        for tok in parts:
            if re.match(r"^\d{4}-\d{2}-\d{2}$", tok):
                return tok
            if re.match(r"^\d{1,2}-[A-Za-z]{3}-\d{4}$", tok):
                try:
                    return pd.to_datetime(tok, errors="coerce").date().isoformat()
                except Exception:
                    return ""
        return ""

    def _parse_opt_type(desc: str, ttype: str) -> str:
        # Prefer transaction type for LEAPs; otherwise use trailing token CALL/PUT
        t = str(ttype or "").upper()
        if "LEAP_CALL" in t: 
            return "CALL"
        if "LEAP_PUT" in t:
            return "PUT"
    ...
    def _parse_action(desc: str) -> str:
        ...
        if w0 in ("BUY", "SELL"):
            return w0.title()
        if w0 == "EXPIRE":
            # expiration closes the position
            return "Buy"
        return w0.title()

    def _parse_qty(desc: str) -> int:
        parts = str(desc or "").split()
        if len(parts) >= 2:
            try:
                return int(abs(float(parts[1])))
            except Exception:
                return 0
        # For "Expire ..." rows, try to infer from OID list count
        m = re.search(r"OID:([0-9,]+)", str(desc or ""), flags=re.I)
        if m:
            try:
    ...
    def _category_from_type(ttype: str) -> str:
        t = str(ttype or "").upper()
        if t.startswith("TRADE_STOCK"):
            return "STOCK"
        if t.startswith("TRADE_LEAP"):
            return "LEAP"
        if t == "OPTION_PREMIUM" or t == "OPTION_EXPIRE":
            return "SHORT OPTION"
        if t in ("DEPOSIT", "WITHDRAWAL", "DIVIDEND", "INTEREST"):
            return "CASH"
        return ""

    def _cash_action(ttype: str) -> str:
        t = str(ttype or "").upper()
        if t == "DEPOSIT":
            return "Deposit"
        if t == "WITHDRAWAL":
    ...
    if st.button("Create Unified Import CSV", type="primary", key="btn_unified_export"):
        # Pull only what we need (avoid schema mismatches)
        qb = (
            supabase.table("transactions")
            .select("transaction_date,type,amount,description,related_symbol,currency")
            .eq("user_id", uid)
            .gte("transaction_date", u_from.isoformat())
            .lte("transaction_date", u_to.isoformat())
            .order("transaction_date", desc=False)
        )
        rows = _fetch_all(qb)
        ...
        for r in rows or []:
            ...
            desc = str(r.get("description") or "")
            sym = str(r.get("related_symbol") or "").upper().strip()
            tx_date = str(r.get("transaction_date") or "").split("T")[0]
            amount = float(r.get("amount") or 0.0)

            cat = _category_from_type(ttype)
            if not cat:
                continue

            fees = _fee_from_desc(desc)
            action = _parse_action(desc)

            if cat == "STOCK":
                qty = _parse_qty(desc)
                price = _parse_money_after_at(desc)
                out.append({
                    "Date": tx_date,
                    "Category": "STOCK",
                    "Action": action,
            ...
            if cat == "LEAP":
                qty = _parse_qty(desc)
                price = _parse_money_after_at(desc)
                exp = _parse_expiration(desc)
                strike = _parse_strike(desc)
                opt_type = _parse_opt_type(desc, ttype)
                out.append({
                    "Date": tx_date,
                    "Category": "LEAP",
                    "Action": action,
            ...
            if cat == "SHORT OPTION":
                exp = _parse_expiration(desc)
                strike = _parse_strike(desc)
                opt_type = _parse_opt_type(desc, ttype) or _parse_opt_type(desc, "")
                # OPTION_PREMIUM rows store net cash impact; reconstruct premium/share
                qty = _parse_qty(desc) or 0
                premium_total = 0.0
                if ttype == "OPTION_EXPIRE":
                    premium_total = 0.0
                    action = "Buy"  # close @ $0
                    ...
                    if amount >= 0:
                        premium_total = abs(amount) + fees
                    else:
                        premium_total = max(0.0, abs(amount) - fees)

                per_share = 0.0
                if qty and qty > 0:
                    per_share = premium_total / (qty * 100.0)

                out.append({
                    "Date": tx_date,
                    "Category": "SHORT OPTION",
                    "Action": action,
                    "Symbol": sym,
                    "Qty": qty,
            ...
            if cat == "CASH":
                # Unified import expects Qty/Price even if Qty isn't used
                out.append({
                    "Date": tx_date,
                    "Category": "CASH",
                    "Action": _cash_action(ttype),
                    "Symbol": sym or "",
                    "Qty": 0,
                    "Price": abs(amount),
                    "Strike": "",
        ...
        dfu = pd.DataFrame(out, columns=["Date","Category","Action","Symbol","Qty","Price","Strike","Expiration","Option Type","Fees"])
        if dfu.empty:
            st.warning("No exportable transactions found in that date range.")
            return

        csv_bytes = dfu.to_csv(index=False).encode("utf-8")
        fname = f"unified_import_{u_from.isoformat()}_to_{u_to.isoformat()}.csv"
        st.download_button("Download Unified Import CSV", data=csv_bytes, file_name=fname, mime="text/csv")
