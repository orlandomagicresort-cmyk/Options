...
def dashboard_page(active_user, view: str = "summary"):
    ...
    if view != "holdings":
        ...
        # --- Quarterly Performance (Rolling Windows) ---
        st.subheader("Quarterly Performance")

        def _rolling_nw_from_weekly_snapshot_pct(n_weeks: int):
            """Return (profit_usd, pct) for trailing n_weeks based on the same flow-normalized weekly return logic
            used in the 52W Profit/Loss row (portfolio_history + DEPOSIT/WITHDRAWAL transactions in USD).
            Includes the current (unfrozen) week from the last snapshot to today.
            """
            try:
                n_weeks = int(n_weeks)
                if n_weeks <= 0:
                ...
                tx_res = supabase.table("transactions").select(
                    "transaction_date, amount, type, currency"
                ).eq("user_id", uid).in_("type", ["DEPOSIT", "WITHDRAWAL"]).execute()
                tx_df = pd.DataFrame(tx_res.data)
                if not tx_df.empty:
                    tx_df = normalize_columns(tx_df)
                ...
                for i in range(len(hist_df)):
                    ...
                    base_capital = prev_eq + net_flow
                    weekly_profit = curr_eq - base_capital
                    weekly_ret = (weekly_profit / base_capital) if base_capital not in (0, 0.0, None) else 0.0

                    weekly_rets.append(float(weekly_ret))
                    weekly_profits.append(float(weekly_profit))

                # Current (unfrozen) week from last snapshot to today
                last_date = hist_df.iloc[-1]["snapshot_date"]
                last_eq = float(hist_df.iloc[-1]["total_equity"])
                cur_flow = 0.0
                if not tx_df.empty:
                ...
                end_i = len(hist_df) - 1
                # include up to the last (n_weeks-1) snapshot weeks + current week = n_weeks periods
                start_k = max(0, end_i - (n_weeks - 2)) if n_weeks >= 2 else end_i

                window_rets = [float(weekly_rets[k]) for k in range(start_k, end_i + 1)] if (end_i >= start_k) else []
                window_rets.append(float(cur_ret))
                ...
                for r in window_rets:
                    if r is None or (isinstance(r, float) and (math.isinf(r) or math.isnan(r))):
                        r = 0.0
                    prod *= (1.0 + float(r))
                pct = float(prod - 1.0) if window_rets else 0.0

                profit = float(sum(weekly_profits[start_k:end_i + 1])) if (end_i >= start_k) else 0.0
                profit += float(cur_profit)
        ...
        qp_rows = []
        for w, lbl in [(13, "13W Rolling"), (26, "26W Rolling"), (39, "39W Rolling"), (52, "52W Rolling")]:
            usd, pct = _rolling_nw_from_weekly_snapshot_pct(w)
            qp_rows.append((lbl, pct, usd, usd * fx))

        qp_html = "<table class='finance-table'><thead><tr><th>Timeframe</th><th>%</th><th>US$</th><th>CA$</th></tr></thead><tbody>"
        for lbl, pct, usd, cad in qp_rows:
            cls = "pos-val" if usd >= 0 else "neg-val"
            qp_html += f"<tr><td>{lbl}</td><td class='{cls}'>{_fmt_pct(pct)}</td><td class='{cls}'>{_fmt_money(usd)}</td><td class='{cls}'>{_fmt_money(cad)}</td></tr>"
        qp_html += "</tbody></table>"
