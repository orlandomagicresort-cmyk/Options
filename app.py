...
def _rolling_nw_from_weekly_snapshot_pct(n_weeks: int):
    ...
    if view != "holdings":
        ...
        for _label, _pct, _usd, _cad in qp_rows:
            ...
            qp_html += f"<tr><td><strong>{_label}</strong></td><td style='text-align:right;'>{_fmt_pct(_pct)}</td><td class='{cls}' style='text-align:right;'><strong>{_fmt_money(_usd)}</strong></td><td class='{cls}' style='text-align:right;'><strong>{_fmt_money(_cad)}</strong></td></tr>"
        qp_html += "</tbody></table>"
