...
# --- Long Options Actions (LEAPs) ---
with st.expander("🧩 Manage Long Options (LEAPs)", expanded=False):
    if leaps_df.empty:
        st.info("No long options to manage.")
    else:
        leap_rows = []
        for _, r in leaps_df.iterrows():
            sym = str(r.get("symbol", "") or r.get("ticker", "")).strip().upper()
            exp = _iso_date(r.get("expiration", r.get("expiration_date", "")))
            strike = float(clean_number(r.get("strike_price", r.get("strike", 0))))
            qty = float(clean_number(r.get("quantity", 0)))
            tnorm = str(r.get("type_norm", r.get("type", ""))).upper().strip()
            # Display type (CALL/PUT)
            tdisp = str(r.get("type_disp", tnorm)).replace("LEAP", "").replace("_", " ").strip()
            if not sym or qty == 0:
                continue
            label = f"{sym} | {format_date_custom(exp)} | Strike: ${strike:,.2f} | Qty: {qty:g} | {tdisp}"
            leap_rows.append((label, sym, exp, strike, qty, tnorm))
        ...
        if not leap_rows:
            ...
            with c1:
                sel_label = st.selectbox("Select Long Option", [x[0] for x in leap_rows], key="long_manage_sel")
                sel = next(x for x in leap_rows if x[0] == sel_label)
                sym, exp_iso, strike, avail_qty, asset_type = sel[1], sel[2], sel[3], sel[4], sel[5]

            with c2:
                action = st.radio(
                    "Action",
                    ["Assign (Exercise into Stock)", "Expire (Close @ $0)", "Roll (Close & New)", "Buy-to-close (Close Long)"],
                    key="long_manage_action",
                    label_visibility="collapsed",
                )

            qty_to_process = st.number_input("Contracts to process", min_value=1.0, max_value=float(avail_qty), value=float(avail_qty), step=1.0, key="long_qty_to_process")

            if action in ["Buy-to-close (Close Long)", "Expire (Close @ $0)"]:
                close_date = st.date_input("Close date", value=date.today(), key="long_close_date")
                close_price = 0.0 if "Expire" in action else st.number_input("Close price ($)", min_value=0.0, value=0.0, step=0.01, format="%.2f", key="long_close_price")
                close_fees = st.number_input("Fees ($)", min_value=0.0, value=0.0, step=0.01, format="%.2f", key="long_close_fees")

                if st.button("Confirm", type="primary", key="long_close_confirm"):
                    txg = str(uuid.uuid4())
                    try:
                        # Closing a long option is a SELL
                        update_asset_position(uid, sym, float(qty_to_process), float(close_price), "Sell", close_date, asset_type=asset_type, expiration=exp_iso, strike=strike, fees=float(close_fees), txg=txg)
                        st.success("Long option closed.")
                        st.rerun()
                    except Exception as e:
                        st.error(f"Could not close long option: {e}")

            elif action == "Assign (Exercise into Stock)":
                ex_date = st.date_input("Exercise date", value=date.today(), key="long_ex_date")
                ex_fees = st.number_input("Fees ($)", min_value=0.0, value=0.0, step=0.01, format="%.2f", key="long_ex_fees")
                st.caption("This will remove the long option and create a stock BUY for 100 shares per contract at the strike price.")

                if st.button("Exercise", type="primary", key="long_ex_confirm"):
                    txg = str(uuid.uuid4())
                    try:
                        # Remove the option position (no proceeds assumed; you can adjust later in ledger if needed)
                        update_asset_position(uid, sym, float(qty_to_process), 0.0, "Sell", ex_date, asset_type=asset_type, expiration=exp_iso, strike=strike, fees=0.0, txg=txg)
                        # Buy stock at strike
                        update_asset_position(uid, sym, float(qty_to_process) * 100.0, float(strike), "Buy", ex_date, asset_type="STOCK", expiration=None, strike=None, fees=float(ex_fees), txg=txg)
                        st.success("Exercised into stock.")
                        st.rerun()
                    except Exception as e:
                        st.error(f"Could not exercise: {e}")

            elif action == "Roll (Close & New)":
                st.caption("Closes the selected long option and opens a new one.")
                r1, r2, r3, r4 = st.columns([1, 1, 1, 1])
                with r1:
                    roll_close_date = st.date_input("Close date", value=date.today(), key="long_roll_close_date")
                with r2:
                    roll_close_price = st.number_input("Close price ($)", min_value=0.0, value=0.0, step=0.01, format="%.2f", key="long_roll_close_price")
                with r3:
                    roll_close_fees = st.number_input("Close fees ($)", min_value=0.0, value=0.0, step=0.01, format="%.2f", key="long_roll_close_fees")
                st.markdown("**New contract**")
                n1, n2, n3 = st.columns([1, 1, 1])
                with n1:
                    new_exp = st.date_input("New expiration", value=(date.today() + timedelta(days=90)), key="long_roll_new_exp")
                with n2:
                    new_strike = st.number_input("New strike", min_value=0.0, value=float(strike), step=0.5, format="%.2f", key="long_roll_new_strike")
                with n3:
                    new_price = st.number_input("New price ($)", min_value=0.0, value=0.0, step=0.01, format="%.2f", key="long_roll_new_price")
                new_fees = st.number_input("Open fees ($)", min_value=0.0, value=0.0, step=0.01, format="%.2f", key="long_roll_new_fees")

                if st.button("Roll", type="primary", key="long_roll_confirm"):
                    txg = str(uuid.uuid4())
                    try:
                        # Close old (SELL)
                        update_asset_position(uid, sym, float(qty_to_process), float(roll_close_price), "Sell", roll_close_date, asset_type=asset_type, expiration=exp_iso, strike=strike, fees=float(roll_close_fees), txg=txg)
                        # Open new (BUY) - keep same option type (CALL/PUT) by reusing asset_type prefix
                        # Convert new exp date to ISO string
                        new_exp_iso = _iso_date(new_exp)
                        update_asset_position(uid, sym, float(qty_to_process), float(new_price), "Buy", roll_close_date, asset_type=asset_type, expiration=new_exp_iso, strike=float(new_strike), fees=float(new_fees), txg=txg)
                        st.success("Rolled long option.")
                        st.rerun()
                    except Exception as e:
                        st.error(f"Could not roll: {e}")
