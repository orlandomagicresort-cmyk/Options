    ...
    for idx, row in enumerate(list(st.session_state["bulk_tx_rows"])):
        ...
        with st.container(border=True):
            ...
            if is_short_sto:
                ...
                exp_dt = c2[1].date_input("Expiry", value=exp_default, key=f"bulk_exp_{rid}")
                strike = c2[2].number_input("Strike", step=0.5, value=float(strike or 0.0), key=f"bulk_strike_{rid}")
                notes = c2[3].text_input("Notes (optional)", value="", key=f"bulk_notes_{rid}")

                r3 = st.columns([1.2, 1.2, 1.0])
                qty = r3[0].number_input("Qty", min_value=1, step=1, value=1, key=f"bulk_qty_{rid}")
                price = r3[1].number_input("Premium", step=0.01, value=0.0, key=f"bulk_price_{rid}")
                ...
                qty = r3[0].number_input("Qty", min_value=1, max_value=max_qty, step=1, value=max_qty, key=f"bulk_qty_{rid}")
                btc_price = r3[1].number_input("BTC Price", step=0.01, value=0.0, key=f"bulk_btc_{rid}")
                new_strike = r3[2].number_input("New Strike", step=0.5, value=float(strike or 0.0), key=f"bulk_new_strike_{rid}")
                new_exp = r3[3].date_input("New Expiry", value=nf, key=f"bulk_new_exp_{rid}")
                new_prem = r3[4].number_input("New Premium", step=0.01, value=0.0, key=f"bulk_new_prem_{rid}")

                fees = st.number_input("Fees", step=0.01, value=0.0, key=f"bulk_fees_{rid}")
                notes = st.text_input("Notes (optional)", value="", key=f"bulk_notes_{rid}")
