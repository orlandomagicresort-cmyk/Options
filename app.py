    ...
    for o in open_shorts:
        try:
            ...
            if not sym or not exp or not typ or oid is None:
                continue
            base_lbl = f"{sym} | {datetime.fromisoformat(exp).strftime('%d-%b-%Y') if exp else exp} | Strike: ${strike:,.2f} | Contracts: {int(contracts)}"
            lbl = base_lbl
            if lbl in seen_labels:
                # Disambiguate duplicates (same ticker/expiry/strike/contracts) by adding type + short id suffix
                lbl = f"{base_lbl} | {typ} | {str(oid)[-4:]}"
            n = 2
            while lbl in seen_labels:
                lbl = f"{base_lbl} | {typ} | {str(oid)[-4:]} | #{n}"
                n += 1
            seen_labels.add(lbl)
            short_opt_map[lbl] = o
