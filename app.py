...
def dashboard_page(active_user, view: str = "summary"):
    ...
    def _rolling_nw_from_weekly_snapshot_pct_local(n_weeks: int):
        """Return (profit_usd, pct) for trailing n_weeks using the SAME flow-normalized weekly method as the 52W row."""
        try:
            n_weeks = int(n_weeks)
            if n_weeks <= 0:
                return 0.0, 0.0
            ...
            # Include current (unfrozen) week from last snapshot to today
            last_date = hist_df.iloc[-1]["snapshot_date"]
            last_eq = float(hist_df.iloc[-1]["total_equity"])
            cur_flow = 0.0
            if not tx_df.empty:
...
def _rolling_nw_from_weekly_snapshot_pct(n_weeks: int):
    ...
    if view != "holdings":
        ...
        qp_defs = [("13W Rolling", 13), ("26W Rolling", 26), ("39W Rolling", 39), ("52W Rolling", 52)]
        qp_rows = []
        for _label, _n in qp_defs:
            _res = _rolling_nw_from_weekly_snapshot_pct_local(_n)
            if _res is None:
                _usd, _pct = 0.0, 0.0
            else:
                _usd, _pct = _res
